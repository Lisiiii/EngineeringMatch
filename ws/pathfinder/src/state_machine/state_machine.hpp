...
private:
    ...
    void update_environment()
    {
        ...
        palstance_msg.data = { rotation_rad_ };
    ...
    {
        rotation_rad_ = msg->data[0] * M_PI / 180.0f;
    }
...
private:
    ...
    float rotation_rad_ = 0.0f;
    float command_id_ = 0.0f;
    float response_id_ = 0.0f;

    std::deque<image::ColorRangeHSV::ColorType> forward_clamped_colors_;
    std::deque<image::ColorRangeHSV::ColorType> backward_clamped_colors_;
...
void inline StateMachine::initialize()
{
    ...
    behaviors_ = {
        [this](DirectionType clamp_direction) {
            ...
            case NOTSTART: {
                ...
                // clamping action
                image::ColorRangeHSV::ColorType front_color = image::ColorRangeHSV::ColorType::CANTIDTIFY;
                while (front_color == image::ColorRangeHSV::ColorType::CANTIDTIFY) {
                    RCLCPP_INFO(this->get_logger(), "Clamping failed, retrying...");
                ...
                if (clamp_direction == DirectionType::FORWARD) {
                    ...
                    backward_clamped_colors_.push_back(front_color);
                }
            ...
            case WORKING:
                if (response_id_ == static_cast<float>((place_direction == DirectionType::FORWARD) ? CommandType::FPLACE : CommandType::BPLACE)) {
                    ...
                    if (place_direction == DirectionType::FORWARD) {
                        ...
                        backward_clamped_colors_.pop_front();
                    }
                }
                return false;
            case COMPLETED:
    ...
    stages_[StageType::GET_A_C_E] = {
        ...
        [this] {
            ...
            auto result = static_cast<int>(stages[current_stage]());
            ...
            if (current_stage >= static_cast<int>(stages.size())) {
                RCLCPP_INFO(this->get_logger(), "Transitioning to stage: COMPLETE_A");
                transitionTo(StageType::COMPLETE_A);
            }
        },
        [] {} // exit
    };
